(function($) {
	...
	function Slide($selector, setScreen, slide_type, slide_num, setAuto, nonAuto_txt, auto_speed, direction){
		this.container = $selector;
		this.list_wrap = this.container.children('ul');
		this.list = this.list_wrap.children('li');
		this.btn_left = this.container.find('.btn_prev');
		this.btn_right = this.container.find('.btn_next');
	...
	Slide.prototype.moveLeft = function(){
		var objThis = this;
		this.moveAmount = $(this.list_wrap).children('li:last-child').outerWidth();

	   $(this.list_wrap).animate({
		   marginLeft: '+=' + objThis.moveAmount +'px'
	   }, 200, function () {
	...
	Slide.prototype.moveRight = function(){
		var objThis = this;
		objThis.moveAmount = $(this.list_wrap).children('li:first-child').outerWidth(true);
	...
	Slide.prototype.moveTop = function(){
		var objThis = this;
		this.moveAmount = $(this.list_wrap).children('li:last-child').outerHeight();

	   $(this.list_wrap).animate({
		   marginTop: '+=' + objThis.moveAmount +'px'
	   }, 200, function () {
		   $(objThis.list_wrap).children('li:last-child').prependTo(objThis.list_wrap);
		   $(objThis.list_wrap).css({
			   marginTop: - objThis.moveAmount
		   });
	   });
	   this.setPage('prev');
	}

	Slide.prototype.moveBottom = function(){
		var objThis = this;
		this.moveAmount = $(this.list_wrap).children('li:first-child').outerHeight(true);
